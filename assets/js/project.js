AOS.init();
/* Project Cards */

const projectcards = document.querySelector(".projectcards");

// Array of object for projects
const projects = [
  {
    title: "Prota-The ROS Bot",
    cardImage: "assets/images/project-page/prota.jpg",
    description: "Autonomous Mobile Robot for Indoor Navigation and SLAM. Worked on ROS, PCB designing and Embedded System Design",
    tagimg: "https://cdn.iconscout.com/icon/free/png-512/react-1-282599.png",
    Previewlink: "https://bit.ly/3JsiHao",
    Githublink: "",
  },
  {
    title: "Strawberry Stacker",
    cardImage: "assets/images/project-page/ss.jpeg",
    description: "This was a swarm drone based project that was developed as a part of eYantra Robotics Competition. Here we had to control two drones simultaneously to perform an optimized pick and place maneuvers in an agriculture field.",
    tagimg: "https://image.flaticon.com/icons/png/512/643/643350.png",
    Previewlink: "https://drive.google.com/file/d/1mQWjyGgMh99IvXbQ5hrY7dlnLXaYYRHZ/view?usp=sharing",
    Githublink: "",
  },
  {
    title: "MiniDrone Spider-Bot",
    cardImage: "assets/images/project-page/drone.jpeg",
    description: "Implemented a small concept of Swarm Robotics in this project, connecting the SpiderBot and the Drone through MQTT.The Drone also had video processing capabilities",
    tagimg: "https://cdn.iconscout.com/icon/free/png-512/react-1-282599.png",
    Previewlink: "https://youtu.be/cVutTdSUZ68",
    Githublink: "",
  },
  {
    title: "Hexacopter",
    cardImage: "assets/images/project-page/hexa.jpg",
    description: "Working on it. It is to be be used as an autonomous delivery drone. Have a plan to implement ROS for autonomous maneuvers.",
    tagimg: "https://miro.medium.com/max/2800/0*U2DmhXYumRyXH6X1.png",
    Previewlink: "https://bit.ly/3JsiHao",
    Githublink: "",
  },
  {
    title: "HID Motion Controller",
    cardImage: "assets/images/project-page/hid.jpeg",
    description: "Its a motion joystick for AR,VR, simulation engines and games. It was tested with both stm32 and esp32 boards.",
    tagimg:
      "data:image/png;base64,",
    Previewlink: "https://youtu.be/azdp5YXCSPE",
    Githublink: "",
  },
  {
    title: "botAlive RoboDog",
    cardImage: "assets/images/project-page/bot.jpg",
    description: "It is an autonomous quadruped robot in the making.",
    tagimg: "https://miro.medium.com/max/816/1*mn6bOs7s6Qbao15PMNRyOA.png",
    Previewlink: "",
    Githublink: "",
  },
  {
    title: "Sanitizing Line Follower Bot",
    cardImage: "assets/images/project-page/line.jpg",
    description:
      "Made this during the start of COVID 19 pandemic for serving necessary items to the needy",
    tagimg: "https://cdn.iconscout.com/icon/free/png-512/react-1-282599.png",
    Previewlink: "http://bit.ly/3sVsRaK",
    Githublink: "",
  },
];

// function for rendering project cards data
//<h4 class="title"><a href="#"><span style="font-size: 20px">${title}</span></a></h4>
//<p class="description"><strong><em><span style="font-size: 20px">${title}</span></strong></em><br>${description}</p>
const showCards = () => {
  let output = "";
  projects.forEach(({ title, cardImage, description, Previewlink, Githublink }) => {
    output += `       
        <div class="column skill-card card" style="margin: 15px"data-aos="zoom-in-up" data-aos-easing="linear" data-aos-delay="300" data-aos-duration="600" >
          <div class="wrapper" style="background: url(${cardImage}) center / cover no-repeat;">
            <div class="header">
            </div>
            <div class="data">
              <div class="content">
                <div class="title-div">
                <h4 class="title"><a href="#">${title}</a></h4>
                </div>
                <section class="decription-content">
                  <p class="description">${description}</p>
                </section>
                <ul class="menu-content"><br>
                  <li><a href="${Previewlink}" class="social-icon"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="35" viewBox="0 0 30 28" fill="none" stroke="#fff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="feather feather-monitor"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"></rect><line x1="8" y1="21" x2="16" y2="21"></line><line x1="12" y1="17" x2="12" y2="21"></line></svg></a></li>
                  <li><a href="${Githublink}" class="social-icon"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="35" viewBox="0 0 30 28" fill="none" stroke="#fff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="feather feather-github"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"></path></svg></a></li>
                </ul>
              </div>
            </div>
          </div>
        </div>`;
  });
  projectcards.innerHTML = output;
};
document.addEventListener("DOMContentLoaded", showCards);

function myFunction() {
  // Declare variables
  var input, button, i, skillcard, card, title;
  input = document.getElementById("myInput").value;
  input = input.toUpperCase();
  skillcard = document.getElementsByClassName("skill-card");
  card = document.getElementsByClassName("card");
  title = document.getElementsByClassName("title");

  // Loop through all list items, and hide those who don't match the search query
  for (i = 0; i < button.length; i++) {
    if (
      button[i].innerHTML.toUpperCase().includes(input) ||
      title[i].innerHTML.toUpperCase().includes(input)
    ) {
      skillcard[i].style.display = "";
      card[i].style.display = "";
    } else {
      skillcard[i].style.display = "none";
      card[i].style.display = "none";
    }
  }
}